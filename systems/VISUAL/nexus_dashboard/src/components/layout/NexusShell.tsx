...
import { MessageSquare, Image as ImageIcon, Video as VideoIcon, Activity, Wand2, LayoutGrid, History, BookOpen, User, Palette } from 'lucide-react';
                ...
                <aside className="w-[320px] h-full flex flex-col gap-4">
                    <section className="bg-[var(--color-stellar-card)]/50 border border-[var(--color-stellar-border)] rounded-[1.8rem] p-6 backdrop-blur-xl flex flex-col gap-6">
                        <div className="flex flex-col gap-1">
                            ...
                            <p className="text-[10px] text-gray-500 tracking-[0.4em] uppercase">Stellar v0.17.0</p>
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 gap-1.5">
                                ...
                                <button onClick={() => setActiveNode('saga')} className={`flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all ${activeNode === 'saga' ? 'bg-purple-500/10 text-purple-400 border border-purple-500/30 shadow-[0_0_15px_rgba(168,85,247,0.1)]' : 'hover:bg-white/5 text-gray-400'}`}>
                                ...
                                <button onClick={() => setActiveNode('vrm')} className={`flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all ${activeNode === 'vrm' ? 'bg-pink-500/10 text-pink-400 border border-pink-500/30' : 'hover:bg-white/5 text-gray-400'}`}>
                                    <User size={16} /> <span className="text-[11px] font-bold uppercase tracking-wider">VRM Avatar</span>
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="mt-auto space-y-4 pt-4 border-t border-white/5">
                            <h3 className="text-[10px] text-gray-500 uppercase tracking-widest font-bold px-2">Sincronía</h3>
                            <div className="bg-black/20 p-4 rounded-2xl border border-white/5 space-y-3">
                                <div className="flex justify-between items-center text-[10px]">
                                    <span className="opacity-50 text-gray-400 uppercase tracking-wider">Frequencia</span>
                                    <span className="text-[var(--color-accent)] font-bold font-mono">161.914 MHz</span>
                                </div>
                    ...
                    <section className="flex-1 bg-[var(--color-stellar-card)]/30 border border-white/5 rounded-[1.8rem] p-6 backdrop-blur-md overflow-hidden flex flex-col">
                        <h3 className="text-[10px] text-gray-400 uppercase tracking-widest mb-4 font-bold">Última Manifestación</h3>
                        <div className="flex-1 rounded-2xl overflow-hidden bg-black/40 border border-white/10 relative group">
                            {latestMedia ? (
                                latestMedia.type === 'video' ? (
                                    <video src={latestMedia.content} autoPlay loop muted className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                                ) : (
                                    <img src={latestMedia.content} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                                )
                            ) : (
                                <div className="h-full flex flex-col items-center justify-center text-gray-700 gap-3">
                                    ...
                                    <span className="text-[9px] tracking-[0.4em] uppercase font-bold text-gray-600">Void Listening</span>
                                </div>
                            )}
                        </div>
                    </section>
                ...
                <section className="w-[340px] h-full flex flex-col gap-4">
                    ...
                    <div className="bg-[var(--color-stellar-card)]/30 border border-white/5 rounded-[1.8rem] p-4 backdrop-blur-md">
                        <div className="grid grid-cols-4 gap-1.5">
                            <button onClick={() => setMode('chat')} className={`p-3 rounded-xl transition-all ${mode === 'chat' ? 'bg-[var(--color-accent)]/20 text-[var(--color-accent)]' : 'bg-white/5 text-gray-500'}`}>
                                <MessageSquare size={16} />
                            </button>
                            <button onClick={() => setMode('imagine')} className={`p-3 rounded-xl transition-all ${mode === 'imagine' ? 'bg-pink-500/20 text-pink-400' : 'bg-white/5 text-gray-500'}`}>
                                <ImageIcon size={16} />
                            </button>
                            <button onClick={() => setMode('vid')} className={`p-3 rounded-xl transition-all ${mode === 'vid' ? 'bg-purple-500/20 text-purple-400' : 'bg-white/5 text-gray-500'}`}>
                                <VideoIcon size={16} />
                            </button>
                            <button onClick={() => setMode('upgrade')} className={`p-3 rounded-xl transition-all ${mode === 'upgrade' ? 'bg-amber-500/20 text-amber-400 animate-pulse' : 'bg-white/5 text-gray-500'}`}>
                                <Wand2 size={16} />
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 bg-black/20 border border-white/5 rounded-[1.8rem] p-6 backdrop-blur-md flex flex-col overflow-hidden">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-[10px] text-gray-500 uppercase tracking-widest font-bold">Chronicle Feed</h3>
                            <button className="p-1 px-2 bg-white/5 rounded-md text-[8px] text-gray-400 border border-white/5">Ver Todo</button>
                        </div>
