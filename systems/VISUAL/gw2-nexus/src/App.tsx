...
import { Brain, RefreshCcw, Cpu, Settings, Package, FlaskConical, Database, Zap, Scale, Target, ArrowLeft } from 'lucide-react';
          ...
          <div className="space-y-2">
            {p.items.map(item => (
              <div key={item.targetId} className="flex justify-between items-center text-[10px]">
                ...
                <span className={item.roi > 0 ? 'text-emerald-400 font-bold' : 'text-red-400 font-bold'}>{item.roi > 0 ? '+' : ''}{item.roi.toFixed(1)}%</span>
              </div>
            ))}
          </div>
        </motion.div>
...
const OperationMode = ({ strategy, materials, wallet, prices, onBack }: { strategy: AnuuStrategy, materials: Record<number, number>, wallet: Record<number, number>, prices: Record<number, MarketItem>, onBack: () => void }) => {
  ...
  const totalGoldCost = (missingT5 * priceT5) + (missingDust * priceDust);
...
function App() {
  ...
  const handleMultiSelect = (strats: AnuuStrategy[], title: string) => {
    ...
    setMultiTitle(title);
    setStatus('GUIDE');
    setThought(`Protocolo de Diversificación "${title}" activo.`);
  };
